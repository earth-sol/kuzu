    ...
    for (auto direction : relGroupEntry->getRelDataDirections()) {
        ...
        directedRelData.emplace_back(
            std::make_unique<RelTableData>(dataFH, memoryManager, shadowFile, *relGroupEntry,
                relEntryInfo->oid, direction, nbrTableID, enableCompression, deSer));
    }
}
...
std::unique_ptr<RelTable> RelTable::loadTable(Deserializer& deSer, const Catalog& catalog,
    ...
    deSer.validateDebuggingInfo(key, "from_table_id");
    deSer.deserializeValue<table_id_t>(fromTableID);
    deSer.validateDebuggingInfo(key, "to_table_id");
    deSer.deserializeValue<table_id_t>(toTableID);
    deSer.validateDebuggingInfo(key, "next_rel_offset");
    deSer.deserializeValue<offset_t>(nextRelOffset);
    const auto relGroupEntry = catalog.getTableCatalogEntry(&DUMMY_TRANSACTION, relGroupID);
    if (!relGroupEntry) {
        throw RuntimeException(
            stringFormat("Load table failed: table {} doesn't exist in catalog.", relGroupID));
    }
    auto innerRelInfo =
        relGroupEntry->cast<RelGroupCatalogEntry>().getRelEntryInfo(fromTableID, toTableID);
    KU_ASSERT(innerRelInfo && innerRelInfo->oid == tableID);
    auto relTable = std::make_unique<RelTable>(relGroupEntry->ptrCast<RelGroupCatalogEntry>(),
        fromTableID, toTableID, storageManager, memoryManager, &deSer);
...
void RelTable::checkpoint(Serializer& ser, TableCatalogEntry* tableEntry) {
    ...
    ser.write<table_id_t>(relGroupID);
    ser.writeDebuggingInfo("from_table_id");
    ser.write<table_id_t>(fromNodeTableID);
    ser.writeDebuggingInfo("to_table_id");
    ser.write<table_id_t>(toNodeTableID);
